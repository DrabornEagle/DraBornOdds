...
export default function dkd_HomeScreen(){
  ...
  return <dkd.Page>
    <dkd.Reveal><dkd.Row><dkd_LivePulse dkd_motion={dkd_store.dkd_motion}/><dkd.Text dkd_size={11} dkd_color={dkd_colors.muted} dkd_bold dkd_style={{letterSpacing:1.4}}>CANLI VERİ MERKEZİ</dkd.Text></dkd.Row><dkd.Text dkd_size={32} dkd_bold dkd_style={{marginTop:10,lineHeight:39}}>Oyunu gör.{'\n'}<dkd_RN.Text style={{color:dkd_colors.lime}}>Veriyi konuştur.</dkd_RN.Text></dkd.Text><dkd.Text dkd_size={14} dkd_color={dkd_colors.muted} dkd_style={{marginTop:9}}>Güncel futbol bülteni ve doğrulanmış market oranları taranır. Analiz motoru mevcut marketleri birlikte değerlendirerek risk profiline uygun seçimleri sıralar.</dkd.Text></dkd.Reveal>
    <dkd_SignalField dkd_motion={dkd_store.dkd_motion} dkd_matches={dkd_matches.length} dkd_oddsReady={dkd_oddsReady}/>
    ...
    {!!dkd_groups.length&&<><dkd.Section dkd_title="Canlı analiz radarı" dkd_subtitle="Normalize edilmiş seçimlerin market ailelerine göre güncel dağılımı."/><dkd.Card dkd_style={{gap:13}}>{dkd_groups.map((dkd_item,dkd_index)=>{const dkd_color=dkd_groupColors[dkd_index]??dkd_colors.lime;return <dkd_RN.View key={dkd_item[0]} style={{gap:6}}><dkd.Row dkd_between><dkd.Text dkd_size={11} dkd_bold>{dkd_item[0]}</dkd.Text><dkd.Text dkd_size={11} dkd_color={dkd_color} dkd_bold>{dkd_item[1]}</dkd.Text></dkd.Row><dkd_AnimatedBar dkd_value={dkd_item[1]} dkd_max={dkd_groupMax} dkd_color={dkd_color} dkd_motion={dkd_store.dkd_motion}/></dkd_RN.View>;})}</dkd.Card></>}
