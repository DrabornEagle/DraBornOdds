...
function dkd_TeamSide({dkd_match,dkd_side}:{dkd_match:dkd_Match;dkd_side:'home'|'away'}){
  ...
  return <dkd_RN.View style={{flex:1,alignItems:'center',gap:8,minWidth:0}}><dkd_TeamLogo dkd_team={dkd_team} dkd_size={52}/><dkd.Text dkd_size={13} dkd_bold dkd_lines={2} dkd_style={{textAlign:'center'}}>{dkd_team.dkd_name}</dkd.Text></dkd_RN.View>;
}
...
export function dkd_LiveMatchCard({dkd_match,dkd_featured}:{dkd_match:dkd_Match;dkd_featured?:boolean}){
  ...
  return <dkd_RN.View style={{borderRadius:24,overflow:'hidden',backgroundColor:dkd_colors.card,borderWidth:1,borderColor:dkd_featured?dkd_accent+'90':dkd_colors.line}}>
    ...
    <dkd_RN.View style={{padding:16,gap:15}}>
      ...
      <dkd_RN.Pressable accessibilityRole="button" accessibilityLabel={`${dkd_match.dkd_home.dkd_name} – ${dkd_match.dkd_away.dkd_name} maç analizini aç`} onPress={()=>dkd_Router.router.push(`/match/${dkd_match.dkd_id}`)} style={{flexDirection:'row',alignItems:'center',gap:10}}><dkd_TeamSide dkd_match={dkd_match} dkd_side="home"/><dkd_RN.View style={{alignItems:'center',gap:4}}><dkd_RN.View style={{width:38,height:38,borderRadius:19,alignItems:'center',justifyContent:'center',backgroundColor:dkd_accent+'18',borderWidth:1,borderColor:dkd_accent+'40'}}><dkd.Text dkd_size={11} dkd_bold dkd_color={dkd_accent}>VS</dkd.Text></dkd_RN.View><dkd.Text dkd_size={9} dkd_color={dkd_colors.muted}>MAÇ</dkd.Text></dkd_RN.View><dkd_TeamSide dkd_match={dkd_match} dkd_side="away"/></dkd_RN.Pressable>
      {!!dkd_display.length?<><dkd.Row dkd_between><dkd.Text dkd_size={10} dkd_color={dkd_colors.muted} dkd_bold>ANALİZ MOTORUNUN ÖNE ÇIKARDIKLARI</dkd.Text><dkd.Text dkd_size={10} dkd_color={dkd_colors.lime}>{dkd_markets.length} seçim</dkd.Text></dkd.Row><dkd.Row dkd_gap={8}>{dkd_display.map((dkd_market,dkd_index)=><dkd_MarketButton key={dkd_market.dkd_key} dkd_match={dkd_match} dkd_market={dkd_market} dkd_index={dkd_index}/>)}</dkd.Row></>:<dkd_RN.View style={{borderRadius:16,padding:13,backgroundColor:dkd_colors.blue+'0E',borderWidth:1,borderColor:dkd_colors.blue+'30'}}><dkd.Row dkd_gap={8}><dkd.Icon dkd_name="clock" dkd_size={16} dkd_color={dkd_colors.blue}/><dkd_RN.View style={{flex:1}}><dkd.Text dkd_size={11} dkd_bold dkd_color={dkd_colors.blue}>FİKSTÜR HAZIR · ORAN BEKLENİYOR</dkd.Text><dkd.Text dkd_size={10} dkd_color={dkd_colors.muted}>Karşılaşma gerçek fikstürde doğrulandı. Eksik oran üretilmeden analiz hattı beklemede.</dkd.Text></dkd_RN.View></dkd.Row></dkd_RN.View>}
