...
export function dkd_marketInsight(dkd_match:dkd_Match,dkd_market:dkd_Market):dkd_AnalysisInsight{
  ...
  const dkd_points=[
    `${dkd_group} içindeki karşılıklı sonuçlar birlikte ele alındı ve bookmaker marjı temizlenerek ${dkd_percent(dkd_probability)} piyasa olasılığı elde edildi.`,
    `${dkd_decimal(dkd_market.dkd_odds)} decimal oran tek başına seçilmedi; normalize olasılık ile oran seviyesi birlikte ölçülerek ${dkd_score}/100 olasılık–oran denge skoru oluştu.`,
    `${dkd_ranked.length} analize uygun seçim aynı maç içinde karşılaştırıldı; “${dkd_market.dkd_label}” denge sıralamasında ${dkd_rank}. sırada.`,
    'Doğrulanmış takım formu, xG veya kadro verisi bulunmadığında bunlar varmış gibi gerekçeye eklenmez; mevcut analiz doğrulanmış piyasa fiyatlamasına dayanır.'
  ];
