...
type dkd_DbSource={dbo_source_id:string;dbo_display_name:string;dbo_last_status:string;dbo_last_run_at:string|null;dbo_last_success_at:string|null;dbo_last_error:string|null;dbo_last_match_count:number};

async function dkd_get<T>(dkd_path:string):Promise<T>{
  const dkd_controller=new AbortController();const dkd_timer=setTimeout(()=>dkd_controller.abort(),12000);
  try{const dkd_response=await fetch(`${dkd_url}/rest/v1/${dkd_path}`,{headers:{apikey:dkd_key,Authorization:`Bearer ${dkd_key}`},signal:dkd_controller.signal});if(!dkd_response.ok)throw new Error(`Canlı veri servisi HTTP ${dkd_response.status}`);return await dkd_response.json() as T;}finally{clearTimeout(dkd_timer);}
}
