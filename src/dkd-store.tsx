...
function dkd_useStateStore() {
  ...
  const dkd_select=(dkd_pick:dkd_Pick)=>{const dkd_existing=dkd_draft.find(dkd_item=>dkd_item.dkd_matchId===dkd_pick.dkd_matchId);if(dkd_existing?.dkd_marketKey===dkd_pick.dkd_marketKey){dkd_setDraft(dkd_prev=>dkd_prev.filter(dkd_item=>dkd_item.dkd_matchId!==dkd_pick.dkd_matchId));dkd_notify('Seçim kupondan çıkarıldı.');return;}if(!dkd_existing&&dkd_draft.length>=6){dkd_notify('Bir kupona en fazla 6 farklı maç ekleyebilirsin.');return;}dkd_setDraft(dkd_prev=>[...dkd_prev.filter(dkd_item=>dkd_item.dkd_matchId!==dkd_pick.dkd_matchId),dkd_pick]);dkd_haptic();dkd_notify(dkd_existing?'Bu maçın kupon seçimi değiştirildi.':'Kupona eklendi.');};
